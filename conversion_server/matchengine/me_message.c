...
// token discount
int push_deal_message(double t, const char *market, order_t *ask, order_t *bid, mpd_t *price, mpd_t *amount, mpd_t *ask_fee, 
    mpd_t *bid_fee, int side, uint64_t id, const char *stock, const char *money, mpd_t *ask_deal_token, mpd_t *bid_deal_token)
{
    json_t *message = json_array();
    json_array_append_new(message, json_real(t));
    ...
    json_array_append_new(message, json_string(ask->token));
    json_array_append_mpd(message, ask->discount);
    json_array_append_mpd(message, ask->token_rate);
    json_array_append_mpd(message, ask->asset_rate);
    json_array_append_mpd(message, ask_deal_token);
    json_array_append_new(message, json_string(bid->token));
    json_array_append_mpd(message, bid->discount);
    json_array_append_mpd(message, bid->token_rate);
    json_array_append_mpd(message, bid->asset_rate);
    json_array_append_mpd(message, bid_deal_token);
