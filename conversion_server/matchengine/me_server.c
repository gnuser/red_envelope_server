...
static bool pledge_conversion(uint64_t id, uint32_t user_id, const char *asset_name, const char *amount,int action)
{
    ...
    log_info("plege reply : %s", json_dumps(reply, 0));
    ...
    log_info("plege status: %s", sds_status);
    ...
    log_info("pledge success");
    return true;
}

int on_cmd_conversion_put_conversion(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    ...
    if (!json_is_string(json_array_get(params, 4)))
        return reply_error_invalid_argument(ses, pkg);
    const char *volume = json_string_value(json_array_get(params, 4));
    ...
    if(1 == role)
    {
        ...
        if ( !pledge_conversion(order_id_start + 1, user_id,stock_name,volume,PLEDGE) )
        {
            return reply_error(ses, pkg, 25, " pledge failed!");
        }
        log_info("push conversion: begin");
        mpd_t* amount    = decimal(volume, market->stock_prec);
        mpd_t* price     = decimal(json_string_value(json_array_get(params, 5)), market->money_prec);
        log_info("push conversion: amount, price");
        json_t *ret_result = NULL;
        int ret =  market_put_conversion_maker(true,&ret_result,market,user_id,amount,price);

        log_info("push conversion: maker");
        mpd_del(amount);
        mpd_del(price);

        if (ret == -1)
        {
            return reply_error(ses, pkg, 11, "ask balance not enough");
        }
        else if (ret == -2)
        {
            return reply_error(ses, pkg, 12, "ask amount too small");
        }
        else if (ret == -4)
        {
            return reply_error(ses, pkg, 17, "ask ate is zero");
        }
        else if (ret < 0)
        {
            log_fatal("market_put_conversion_order_maker fail: %d", ret);
            return reply_error_internal_error(ses, pkg);
        }
        log_info("push conversion: append log");
        append_operlog("put_conversion", params);
        ret = reply_result(ses, pkg, ret_result, true);
        json_decref(ret_result);
        return ret;
    }
    else if(2 == role)//put_conversion(role,user_id,stock,money,volume,conversion_id)	  --> taker调用形式
    {
        // conversion_id
        if (!json_is_integer(json_array_get(params, 1)))
            return reply_error_invalid_argument(ses, pkg);
        uint64_t conversion_id  = json_integer_value(json_array_get(params, 5));
        mpd_t *amount = decimal(volume, market->stock_prec);
        order_t *order = market_get_order(market, conversion_id);
        if (order == NULL)
        {
            return reply_error(ses, pkg, 14, "conversion id not found");
        }

        if (order->user_id == user_id)
        {
            return reply_error(ses, pkg, 15, "self-closing");
        }
	...
	mpd_rescale(pledge_money_amount, pledge_money_amount, -9, &mpd_ctx);
        ...
        {
            mpd_del(pledge_money_amount);
            return reply_error(ses, pkg, 25, "pledge failed");
        }
        mpd_del(pledge_money_amount);
        log_info("conversion taker: put conversion");
        json_t *ret_result = NULL;
        int ret = market_put_conversion_taker(true, &ret_result, market, user_id, order, amount, stock_name, money_name);
        mpd_del(amount);

        if (ret == 20)
        {
            return reply_error(ses, pkg, ret, "ask stock withdraw failed");
        }
        else if (ret == 21)
        {
            return reply_error(ses, pkg, ret, "ask money update failed");
        }
        else if (ret == 22)
        {
            return reply_error(ses, pkg, ret, "bid stock update failed");
        }
        else if (ret == 23)
        {
            return reply_error(ses, pkg, ret, "bid money withdraw failed");
        }
	else if( ret == 26)
	{
	    return reply_error(ses, pkg, ret, "back pledge fail");
	}
        else if (ret != 0)
        {
            return reply_error(ses, pkg, ret, "internal error");
        }

        log_info("conversion taker: append log");
        append_operlog("put_conversion", params);
        ret = reply_result(ses, pkg, ret_result, true);
        json_decref(ret_result);
        return ret;
...
static int on_cmd_order_cancel(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    ...
    */
    ...
    bool ret_pledge = pledge_conversion(order->id, order->user_id,market->stock,stock_volume,UNFREEZE);
...
#endif
//#ifdef CONVERSION
        case CMD_CONVERSION_PUT_CONVERSION:
            if (is_operlog_block() || is_history_block() || is_message_block()) {
                log_fatal("service unavailable, operlog: %d, history: %d, message: %d",
                          is_operlog_block(), is_history_block(), is_message_block());
                reply_error_service_unavailable(ses, pkg);
            ...
            log_trace("from: %s put conversion, sequence: %u params: %s", nw_sock_human_addr(&ses->peer_addr), pkg->sequence, params_str);
            ret = on_cmd_conversion_put_conversion(ses, pkg, params);
            if (ret < 0) {
                log_error("on_cmd_conversion_put_conversion %s fail: %d", params_str, ret);
            }
            break;
        case CMD_CONVERSION_QUERY:
            log_trace("from: %s cmd order query, sequence: %u params: %s", nw_sock_human_addr(&ses->peer_addr), pkg->sequence, params_str);
            ret = on_cmd_conversion_query(ses, pkg, params);
            if (ret < 0) {
                log_error("on_cmd_conversion_query %s fail: %d", params_str, ret);
            }
            break;
