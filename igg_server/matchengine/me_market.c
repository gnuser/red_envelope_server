...
static bool pledge_order( uint32_t user_id, const char *asset_name, mpd_t* amount)
{
    if (NULL == balance_sub(user_id,BALANCE_TYPE_AVAILABLE,asset_name,amount))
        return false;
    balance_add(user_id,BALANCE_TYPE_PLEDGE,asset_name,amount);
    return true;
}

int market_add_order(bool real, json_t **result, market_t *m, uint32_t user_id, mpd_t *amount, mpd_t *price, bool sellstock)
{
    uint32_t side = MARKET_ORDER_SIDE_ASK;

    if(!sellstock)
        side = MARKET_ORDER_SIDE_BID;
    if (side == MARKET_ORDER_SIDE_ASK )
    {
        if( !pledge_order(user_id,m->stock,amount))
        {
            return -__LINE__;
        }
    }
    ...
    {
        if( !pledge_order(user_id,m->money,amount))
        {
            return -__LINE__;
        }
    }
    ...
    {
        ret = append_order_history(order);
        log_info("add order :  push message");
        push_order_message(ORDER_EVENT_PUT, order, m);
        log_info("add order: get order info");
        *result = get_order_info(order);
    }
...
static json_t* execute_order(bool real, market_t *m, order_t *ask_order, order_t *bid_order)
{
    mpd_t *match_stock = mpd_new(&mpd_ctx);

    mpd_mul(match_stock, bid_order->price, ask_order->deal_stock, &mpd_ctx);

    int cmp_ask = mpd_cmp(ask_order->left, mpd_zero,& mpd_ctx);
    int cmp_bid = mpd_cmp(bid_order->left, mpd_zero,& mpd_ctx);

    if ( cmp_ask == 0 && cmp_bid == 0)
    {
        if (real)
        {

            append_order_history(ask_order);
            push_order_message(ORDER_EVENT_FINISH, ask_order, m);
            append_order_history(bid_order);
            ...
            //    mpd_t *price, mpd_t *amount, mpd_t *deal, mpd_t *ask_fee, mpd_t *bid_fee, mpd_t *ask_deal_token, mpd_t *bid_deal_token);
            append_order_deal_history(ask_order->update_time, deal_id, ask_order, MARKET_ORDER_SIDE_ASK,
                                      bid_order,MARKET_ORDER_SIDE_ASK, ask_order->price, ask_order->amount,
                                      ask_order->deal_stock, ask_fee, bid_fee, ask_deal_token, bid_deal_token);
        }

        order_finish(real,m,ask_order);
        order_finish(real,m,bid_order);
        char * deal_stock = mpd_to_sci(match_stock,0);

        return conversion_success(deal_stock);
    }
    else if (cmp_bid > 0)
    {
        ...
        if (real)
        {
            ...
            push_order_message(ORDER_EVENT_UPDATE, bid_order, m);
            ...
            append_order_deal_history(bid_order->update_time, deal_id, ask_order, MARKET_ORDER_SIDE_ASK,
                                      bid_order,MARKET_ORDER_SIDE_ASK, ask_order->price, ask_order->amount,
                                      ask_order->deal_stock, ask_fee, bid_fee, ask_deal_token, bid_deal_token);

        }
        char * deal_stock = mpd_to_sci(ask_order->deal_stock,0);
        order_finish(real,m,ask_order);
        return conversion_success(deal_stock);
    ...
    else
    {
        ...
        {
            append_order_history(ask_order);
            push_order_message(ORDER_EVENT_UPDATE, ask_order, m);
            append_order_history(bid_order);
            push_order_message(ORDER_EVENT_FINISH, bid_order, m);
            uint64_t deal_id = ++deals_id_start;
            mpd_t *ask_fee = mpd_new(&mpd_ctx);
...
static int settle_order(bool real, market_t *m, order_t *ask_order, order_t *bid_order)
{
    mpd_t *match_stock = mpd_new(&mpd_ctx);
    mpd_mul(match_stock, bid_order->price, bid_order->left, &mpd_ctx);
    int cmp = mpd_cmp(ask_order->left, match_stock,& mpd_ctx);

    if(cmp <= 0 )
    {

        mpd_t* ask_pledge = balance_get(ask_order->user_id,BALANCE_TYPE_PLEDGE,m->stock);
        if (mpd_cmp(ask_pledge, ask_order->left, &mpd_ctx) < 0)
        {
             mpd_del(match_stock);
            return -103;
        }

        mpd_t *deal_money = mpd_new(&mpd_ctx);
        mpd_div(deal_money,ask_order->left, ask_order->price,&mpd_ctx);
        mpd_t* bid_pledge = balance_get(bid_order->user_id,BALANCE_TYPE_PLEDGE,m->money);
        if (mpd_cmp(bid_pledge, deal_money, &mpd_ctx) < 0)
        {
            mpd_del(match_stock);
            return -104;
        }

        balance_sub(ask_order->user_id,BALANCE_TYPE_PLEDGE,m->stock,ask_order->left);
        balance_add(ask_order->user_id,BALANCE_TYPE_SETTLE,m->money,deal_money);

        balance_sub(bid_order->user_id,BALANCE_TYPE_PLEDGE,m->money,deal_money);
        balance_add(bid_order->user_id,BALANCE_TYPE_SETTLE,m->stock,ask_order->left);

        mpd_sub(ask_order->deal_stock,ask_order->left,mpd_zero,&mpd_ctx);
        mpd_sub(ask_order->left,ask_order->left,ask_order->left,&mpd_ctx );
        mpd_sub(bid_order->left,bid_order->left,deal_money,&mpd_ctx);
    ...
    else
    {
        ...
        mpd_t* ask_pledge = balance_get(ask_order->user_id,BALANCE_TYPE_PLEDGE,m->stock);
        if (mpd_cmp(ask_pledge, match_stock, &mpd_ctx) < 0)
        {
            mpd_del(match_stock);
            return -103;
        }
        ...
        mpd_t* bid_pledge = balance_get(bid_order->user_id,BALANCE_TYPE_PLEDGE,m->money);
        if (mpd_cmp(bid_pledge, bid_order->left, &mpd_ctx) < 0)
        {
            mpd_del(match_stock);
            return -104;
        }

        balance_sub(ask_order->user_id,BALANCE_TYPE_PLEDGE,m->stock,match_stock);
        balance_add(ask_order->user_id,BALANCE_TYPE_SETTLE,m->money,bid_order->left);

        balance_sub(bid_order->user_id,BALANCE_TYPE_PLEDGE,m->money,bid_order->left);
        balance_add(bid_order->user_id,BALANCE_TYPE_SETTLE,m->stock,match_stock);

        mpd_sub(ask_order->deal_stock,match_stock,mpd_zero,&mpd_ctx);
        mpd_sub(ask_order->left,ask_order->left,match_stock,&mpd_ctx);
        mpd_sub(bid_order->left,bid_order->left,bid_order->left,&mpd_ctx);
...
{
    order_t* ask_order = market_get_order(m,ask_orderid);
    order_t* bid_order = market_get_order(m,bid_orderid);

    if(ask_order == NULL || bid_order == NULL)
    {
        return -100;
    }

    if (ask_order->side  != MARKET_ORDER_SIDE_ASK || bid_order->side  != MARKET_ORDER_SIDE_BID)
    {
        return -101;
    }
    ...
    int ret = settle_order(real,m,ask_order,bid_order);

    if(ret != 0)
    {
        return ret;
    }
    *result = execute_order(real,m,ask_order,bid_order);
    return 0;
}
