...
static sds on_cmd_balance_summary(const char *cmd, int argc, sds *argv)
{
    ...
    for (size_t i = 0; i < settings.asset_num; ++i) {
        balance_status(settings.assets[i].name, total, &available_count, available, &freeze_count, freeze, &pledge_count, pledge);
        char *total_str = mpd_to_sci(total, 0);
        char *available_str = mpd_to_sci(available, 0);
        char *freeze_str = mpd_to_sci(freeze, 0);
        char *pledge_str = mpd_to_sci(pledge, 0);
        reply = sdscatprintf(reply, "%-16s %-30s %-10zu %-30s %-10zu %-30s %-10zu %-30s\n", settings.assets[i].name,
                total_str, available_count, available_str, freeze_count, freeze_str, pledge_count, pledge_str);
        free(total_str);
        free(available_str);
        free(freeze_str);
    }
